...
// Récupérer les informations de l'enfant et du groupe avec une requête simplifiée
$stmt = $pdo->prepare("
    SELECT 
        e.id_enfant,
        e.nom, 
...
// Récupération séparée de l'éducatrice responsable pour debug
$educatrice_nom = null;
$educatrice_prenom = null;

if ($child && !empty($child['id_groupe']) && !empty($child['annee_scolaire'])) {
    // Requête séparée pour récupérer l'éducatrice
    $stmt = $pdo->prepare("
        SELECT 
    ...
    if ($educatrice) {
        ...
        // Fallback: chercher une éducatrice pour ce groupe sans condition d'année
        $stmt = $pdo->prepare("
            SELECT 
                p.nom AS educatrice_nom, 
                p.prenom AS educatrice_prenom
...
// Récupérer la prochaine activité du groupe de l'enfant
...
// Récupérer les activités du jour pour le groupe de l'enfant
$today_activities = [];
if (!empty($child['id_groupe'])) {
    $stmt = $pdo->prepare("
        SELECT a.nom_activite, ga.date_d_activite
...
// Récupérer les absences de la semaine pour l'enfant
$absences = [];
...
// Debug: ajouter ces lignes temporairement pour diagnostiquer
/*
echo "Debug Info:<br>";
echo "ID Groupe: " . ($child['id_groupe'] ?? 'NULL') . "<br>";
echo "Année scolaire: " . ($child['annee_scolaire'] ?? 'NULL') . "<br>";
echo "Éducatrice trouvée: " . ($educatrice_prenom ?? 'NULL') . " " . ($educatrice_nom ?? 'NULL') . "<br>";
*/
?>
